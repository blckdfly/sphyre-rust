...
pub async fn list_consents(
    ...
    let consents = list_user_consents(&state.db, &user.id).await
...
pub async fn get_consent(
    ...
    let consent = get_consent_by_id(&state.db, &consent_id, &user.id).await
...
pub async fn create_consent(
    ...
    let consent = create_new_consent(&state.db, blockchain, &user.id, consent_input).await
        .map_err(|e| ApiError::InternalError(e.to_string()))?;
...
pub async fn update_consent(
    ...
    let updated_consent = update_consent_by_id(&state.db, blockchain, &consent_id, &user.id, consent_input).await
        .map_err(|e| match e.to_string().as_str() {
            "Consent not found" => ApiError::NotFound(e.to_string()),
...
pub async fn revoke_consent(
    ...
    let revoked = revoke_consent_by_id(&state.db, blockchain, &consent_id, &user.id).await
        .map_err(|e| match e.to_string().as_str() {
            "Consent not found" => ApiError::NotFound(e.to_string()),
